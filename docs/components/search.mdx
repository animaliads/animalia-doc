...
O resultado da busca é exibido através do componente [`listbox`](./listbox.mdx), que pode ser acionado dinâmicamente na variação [`active`](#comportamento) ou manualmente na variação [`trigger`](#comportamento).
...
- Se possível, forneça sugestões de pesquisa, seja em um [`helptext`](./textfield.mdx#anatomia) ou sugestão de pesquisa que é um _autocomplete_. Isso ajuda os usuários a encontrar o que estão procurando, especialmente se os itens pesquisáveis forem complexos.
