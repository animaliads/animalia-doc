...
import StepperAnatomy from './img/stepper/stepper-anatomy.png';
import StepperVertical from './img/stepper/stepper-vertical.png';
import StepperOverText from './img/stepper/stepper-overtext.png';
import StepperStatus from './img/stepper/stepper-status.png';
import StepperHorizontalSpacing from './img/stepper/stepper-horizontal-spacing.png';
import StepperVerticalSpacing from './img/stepper/stepper-vertical-spacing.png';
...
O componente preve também duas variações de orientação: `vertical` e `horizontal`. Em ambas, os elementos possuem as mesmas características especificadas anteriormente, mudando apenas o posicionamento dos elementos e espaçamentos.
...
**Label:** contém um `padding` de `2rem` à direita e `0.25rem` ao topo.

**Circle** (`done`, `next`)**:** contém um `padding` de `0.25rem` ao topo e abaixo.

<img
  src={StepperHorizontalSpacing}
  alt="Imagem representando o espaçamento do componente stepper na horizontal, apontando os espaços na direita e acima do label, como também do circulo no topo e abaixo."
/> 

1. **Padding right (label)** 
2. **Padding top (label)** 
3. **Padding (circle)** 

#### Vertical

**Label:** contém um `padding` de `1rem` à esquerda e `0.25rem` ao topo e abaixo.

**Circle** (`done`, `next`)**:** contém um `padding` de `0.25rem` em todos os lados.

<img
  src={StepperVerticalSpacing}
  alt="Imagem representando o espaçamento do componente stepper na vertical, apontando os espaços na esquerda, acima e abaixo do label, como também em torno do circulo."
/>

1. **Padding left (label)** 
2. **Padding top, bottom (label)** 
3. **Padding (circle)** 
